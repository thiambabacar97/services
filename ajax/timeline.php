...
switch ($_REQUEST['action']) {
   ...
   case "viewsubitem":
      ...
      if ($_REQUEST['type'] == "Solution") {
         ...
         $fup = new PluginServicesITILFollowup();
         $id = isset($_REQUEST['id']) && (int)$_REQUEST['id'] > 0 ? $_REQUEST['id'] : null;
         if ($id) {
            $fup->getFromDB($id);
         }
      ...
      && isset($_REQUEST["id"])
      && $parent->getFromDB($_REQUEST[$parent->getForeignKeyField()]) && $_REQUEST['type'] == 'Document_Item' ) {
         ...
         PluginServicesDocument_Item::showAddFormForItem($parent, '');
