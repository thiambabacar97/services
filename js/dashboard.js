...
var Dashboard = {
   ...
   display: function(params) {
      ...
      // add itemtype
      $("#dashboard-"+options.rand+" .toolbar .add-item").click(function() {
         Dashboard.addItem();
      });
      $(document).on('submit', '.display-add-item-form', function(event) {
         event.preventDefault();

         $(".ui-dialog-content").dialog("close");
         var button    = $(this);
         var form_data = {};
         $.each(button.closest('.display-add-item-form').serializeArray(), function() {
            form_data[this.name] = this.value;
         });
      ...
      $(document).on('select2:select', '.display-widget-form select[name=card_id]', function(event) {
         ...
         console.log(Dashboard.all_cards[selected]);
   ...
   addForm: function() {
      ...
      $('<div title="'+__("Add a new dashboard")+'"></div>')
         .load(CFG_GLPI.root_doc+"/plugins/services/ajax/dashboard.php", {
            action: 'add_new',
         }, function() {
            $(this).dialog({
               width: '100%',
   ...
   /**
    * Display form to add a new Itemtype
    */
   addItem: function() {
      $(".ui-dialog-content").dialog("close");
      $('<div title="'+__("Add an item")+'"></div>')
         .load(CFG_GLPI.root_doc+"/ajax/dashboard.php", {
            action: 'add_item',
         }, function() {
            $(this).dialog({
