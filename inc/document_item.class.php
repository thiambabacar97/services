...
**/
class PluginServicesDocument_Item extends Document_Item{
   ...
   **/
   ...
   function postForm($post){
      ...
      if (isset($post["add"])) {
         ...
         $doc->check(-1, CREATE, $post);
         if (isset($post['_filename']) && is_array($post['_filename'])) {
            $fic = $post['_filename'];
            $tag = $post['_tag_filename'];
            $prefix = $post['_prefix_filename'];
            foreach (array_keys($fic) as $key) {
               $post['_filename']        = [$fic[$key]];
               $post['_tag_filename']    = [$tag[$key]];
               $post['_prefix_filename'] = [$prefix[$key]];

               if ($newID = $doc->add($post)) {
                  Event::log($newID, "documents", 4, "login",
                  sprintf(__('%1$s adds the item %2$s'), $_SESSION["glpiname"], $doc->fields["name"]));
               }
            }
            if ($_SESSION['glpibackcreated'] && (!isset($post['itemtype']) || !isset($post['items_id']))) {
               Html::redirect($doc->getLinkURL());
            }
         } else if ($newID = $doc->add($post)) {
            Event::log($newID, "documents", 4, "login",
                     sprintf(__('%1$s adds the item %2$s'), $_SESSION["glpiname"], $doc->fields["name"]));
            // Not from item tab
            if ($_SESSION['glpibackcreated'] && (!isset($post['itemtype']) || !isset($post['items_id']))) {
   ...
   */
    ...
    * @return boolean
   **/
   static function showAddFormForItecm(CommonDBTM $item, $withtemplate = 0, $options = []) {
      global $DB, $CFG_GLPI;
      ...
      if ($item->canAddItem('Document')
         ...
         echo "<form name='documentitem_form".$params['rand']."' id='documentitem_form".
               $params['rand']."' method='post' action='".self::getFormURL(true).
         ...
         PluginServicesHtml::file(['multiple' => true]);
        echo"</tr>";
         echo "</table>";
         
         echo"<div class='d-flex justify-content-end bg-white'>";
         echo "<input type='submit' name='add' value=\""._sx('button', 'Add')."\" class='submit'>";
         ...
         if (Document::canView()
            ...
            echo "<form name='document_form".$params['rand']."' id='document_form".$params['rand'].
                  "' method='post' action='".self::getFormURL()."'>";
            echo "<table class='tab_cadre_fixe'>";
            echo "<tr class='tab_bg_1'>";
   ...
   static function showAddFormForItem(CommonDBTM $item, $withtemplate = 0, $options = []) {
      ...
      if ($item->canAddItem('Document')
         ...
         if ($item->getType() == 'Document') {
            ...
            echo "<form id='mainform' class='m-form m-form--fit m-form--label-align-right ' name='documentitem_form".$params['rand']."' id='documentitem_form".
                  $params['rand']."' method='post' action='".Toolbox::getItemTypeFormURL('Document').
                  "' enctype=\"multipart/form-data\">";
                  echo"<div class='form-group m-form__group row mb-3'>";
                        ...
                        echo"<div class=' d-flex justify-content-center mt-2'>";
                           ...
                           <button type="submit" name="add" class="btn btn-default">
                              <span>
                                 ...
                                 <span>'._sx('button', 'Add a new file').'</span>
                              </span>
                           </button>';
                        echo"</div>";
                     echo"</div>";
    ...
    * @param $options        array
    */
   static function showListForItems(CommonDBTM $item, $withtemplate = 0, $options = []) {
      global $DB;
