...
class PluginServicesDisplayPreference  extends DisplayPreference {
  ...
  **/
  function showFormPerso($target, $itemtype) {
    ...
    if ($numrows == 0) {
        ...
        echo "&nbsp;&nbsp;<button onclick=\"submitForm(event, 'activate')\"   id='activate' type='button' class='btn btn-secondary btn-sm'>".__('Create')."</button>";
        ...
        echo "<table style='width:100%; margin-top:0;' class='m-0 tab_cadrehov table table-striped m-table'><tr style='text-align: left'><th colspan='4'>";
        echo "<form id='disable'>";
        echo "<input type='hidden' name='itemtype' value='$itemtype'>";
        echo "<input type='hidden' name='users_id' value='$IDuser'>";
        echo "<input type='hidden' name='disable'>";
        echo __('Select default items to show')."<span class='small_space'>";
        echo "&nbsp;&nbsp;<button onclick=\"submitForm(event, 'disable')\"  type='button' class='btn btn-secondary btn-sm'>".__('Delete')."</button>";
        PluginServicesHtml::closeForm();

        echo "</th></tr>";
        echo "<tr class='tab_bg_1' style='text-align: left'><td colspan='4' class='center'>";
        echo "<form id='addformPerso'>";
        echo "<input type='hidden' name='itemtype' value='$itemtype'>";
        echo "<input type='hidden' name='users_id' value='$IDuser'>";
        ...
        if ($values) {
          ...
          echo"<button onclick=\"submitForm(event, 'addformPerso')\" class='btn m-btn--radius btn-md  btn-info ml-2' >Ajouter</button>";
          echo "</span>";
        }
        PluginServicesHtml::closeForm();
        echo "</td></tr>\n";
          ...
          while ($data = $iterator->next()) {
              if (($data["num"] !=1) && isset($searchopt[$data["num"]])) {
                ...
                if ($i != 0) {
                    ...
                    echo "<form id='upformperso".$data["id"]."'>";
                    echo "<input type='hidden' name='id' value='".$data["id"]."'>";
                    echo "<input type='hidden' name='users_id' value='$IDuser'>";
                    echo "<input type='hidden' name='itemtype' value='$itemtype'>";
                    echo "<input type='hidden' name='up'>";
                    echo "<button onclick=\"submitForm(event, 'upformperso".$data["id"]."')\" ".
                        " title=\"".__s('Bring up')."\"".
                ...
                if ($i != ($numrows-1)) {
                    echo "<td class='center middle'>";
                    echo "<form id='downformperso".$data["id"]."'>";
                    echo "<input type='hidden' name='id' value='".$data["id"]."'>";
                    echo "<input type='hidden' name='users_id' value='$IDuser'>";
                    echo "<input type='hidden' name='itemtype' value='$itemtype'>";
                    echo "<input type='hidden' name='down'>";
                    echo "<button onclick=\"submitForm(event, 'downformperso".$data["id"]."')\" ".
                ...
                if (!isset($searchopt[$data["num"]]["noremove"]) || $searchopt[$data["num"]]["noremove"] !== true) {
                    echo "<td class='center middle'>";
                    echo "<form  id='purgformperso".$data["id"]."'>";
                    echo "<input type='hidden' name='id' value='".$data["id"]."'>";
                    echo "<input type='hidden' name='users_id' value='$IDuser'>";
                    echo "<input type='hidden' name='itemtype' value='$itemtype'>";
                    echo "<input type='hidden' name='itemtype' value='$itemtype'>";
                    echo "<input type='hidden' name='purge'>";
                    echo "<button onclick=\"submitForm(event, 'purgformperso".$data["id"]."')\" ".
    ...
    $jsScript = '
      var tokenurl = "plugins/services/ajax/generatetoken.php";
      function submitForm(e, domId) { 
        e.preventDefault();
          var params = $("#"+domId).serializeArray();
          $.ajax({ url: tokenurl, type: "GET", datatype: "json", 
            success: function(data) { 
              params[params.length] = { name: "_glpi_csrf_token", value:data };
              $.post("'.$target.'", params, function(data){
                  loadPageModal("'.$target.'?itemtype='.$itemtype.'");
              });   
            }
          });
      }
  ...
  **/
  function showFormGlobal($target, $itemtype) {
    ...
    echo "<table style='width:100%; margin-top:0;' class='m-0 tab_cadrehov table table-striped m-table'><tr></tr>\n";

    if ($global_write) {
        $already_added = self::getForTypeUser($itemtype, $IDuser);
        echo "<tr class='tab_bg_1' ><td colspan='4' class=' class='float-left''>";
        ...
        if ($values) {
          PluginServicesDropdown::showFromArray('num', $values);
          echo "<span class='small_space'>";
          echo"<button onclick=\"submitForm(event, 'addform')\"   id='addform' class='btn m-btn--radius btn-md  btn-info ml-2'  >Ajouter</button>";
          echo "</span>";
        }
        PluginServicesHtml::closeForm();
        echo "</td></tr>";
        ...
        while ($data = $iterator->next()) {
          ...
          if (($data["num"] != 1)
              ...
              if ($global_write) {
                if ($i != 0) {
                    echo "<td class='center middle'>";
                    echo "<form id='upform".$data["id"]."'>";
                    echo "<input type='hidden' name='id' value='".$data["id"]."'>";
                    echo "<input type='hidden' name='users_id' value='$IDuser'>";
                    echo "<input type='hidden' name='itemtype' value='$itemtype'>";
                    echo "<input type='hidden' name='up'>";
                    echo "<button  onclick=\"submitForm(event, 'upform".$data["id"]."')\"   id='".$data["id"]."'  ".
                        " title=\"".__s('Bring up')."\"".
                        " class='unstyled pointer'><i class='fa fa-arrow-up'></i></button>";
                    PluginServicesHtml::closeForm();
                    echo "</td>";
                ...
                if ($i != ($numrows-1)) {
                    echo "<td class='center middle'>";
                    echo "<form id='downform".$data["id"]."'>";
                    echo "<input type='hidden' name='id' value='".$data["id"]."'>";
                    echo "<input type='hidden' name='users_id' value='$IDuser'>";
                    echo "<input type='hidden' name='itemtype' value='$itemtype'>";
                    echo "<input type='hidden' name='down'>";
                    echo "<button  onclick=\"submitForm(event, 'downform".$data["id"]."')\"   id='".$data["id"]."' ".
                        " title=\"".__s('Bring down')."\"".
                        " class='unstyled pointer'><i class='fa fa-arrow-down'></i></button>";
                    PluginServicesHtml::closeForm();
                    echo "</td>";
                ...
                if (!isset($searchopt[$data["num"]]["noremove"]) || $searchopt[$data["num"]]["noremove"] !== true) {
                    echo "<td class='center middle'>";
                    echo "<form id='purgeform".$data["id"]."' >";
                    echo "<input type='hidden' name='id' value='".$data["id"]."'>";
                    echo "<input type='hidden' name='users_id' value='$IDuser'>";
                    echo "<input type='hidden' name='itemtype' value='$itemtype'>";
                    echo "<input type='hidden' name='purge'>";
                    echo "<button   onclick=\"submitForm(event, 'purgeform".$data["id"]."')\"   id='".$data["id"]."' ".
                          " title=\""._sx('button', 'Delete permanently')."\"".
                          " class='unstyled pointer'><i class='fa fa-times-circle'></i></button>";
                    PluginServicesHtml::closeForm();
                    echo "</td>\n";
  ...
  **/
  static function showForUser($users_id) {
    ...
    if (count($iterator) > 0) {
        ...
        echo PluginServicesHtml::hidden('users_id', ['value'                 => $users_id,
                                            'data-glpicore-ma-tags' => 'common']);
        echo "<table class='tab_cadre_fixe'>";
        echo "<tr>";
        echo "<th width='10'>";
        echo PluginServicesHtml::getCheckAllAsCheckbox('mass'.__CLASS__.$rand);
        echo "</th>";
        echo "<th colspan='2'>"._n('Type', 'Types', 1)."</th></tr>";
        while ($data = $iterator->next()) {
          echo "<tr class='tab_bg_1'><td width='10'>";
          PluginServicesHtml::showMassiveActionCheckBox(__CLASS__, $data["itemtype"]);
          echo "</td>";
  ...
  **/
  ...
  public function teste($t){
    echo'
    <ul class="nav nav-tabs  m-tabs-line m-tabs-line--success" role="tablist">
      <li class="nav-item m-tabs__item">
        <a class="nav-link m-tabs__link active" data-toggle="tab" href="#m_tabs_2_1" role="tab">Vue globale</a>
      </li>
      <li class="nav-item m-tabs__item">
        <a class="nav-link m-tabs__link" data-toggle="tab" href="#m_tabs_2_3" role="tab">Vue personnelle</a>
      </li>
    </ul>';

    echo'<div class="tab-content">
      <div class="tab-pane active" id="m_tabs_2_1" role="tabpanel">'; $this->showFormGlobal(Toolbox::getItemTypeFormURL('PluginServicesDisplayPreference', true), $t["itemtype"]); echo'</div>';
      echo'<div class="tab-pane" id="m_tabs_2_3" role="tabpanel">'; $this->showFormPerso(Toolbox::getItemTypeFormURL('PluginServicesDisplayPreference', true), $t["itemtype"]); echo'</div>';
    echo'</div>';
    echo'<style>
      .nav-item {
          border-left: none;
