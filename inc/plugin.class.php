...
class PluginServicesPlugin extends CommonDBTM {
   ...
   static function getSpecificValueToSelect($field, $name = '', $values = '', array $options = []) {
      ...
      switch ($field) {
         case 'state':
            ...
            return PluginServicesDropdown::showFromArray($name, $tab, $options);
            break;
      }
   }
   ...
   function postForm ($post){
      ...
      if (method_exists($plugin, $post['action'])) {
         call_user_func([$plugin, $post['action']], $post['id']);
      } else {
         echo "Action ".$post['action']." undefined";
      }
         Html::back();
      }
   }
