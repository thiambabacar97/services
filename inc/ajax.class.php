...
class PluginServicesAjax {
   ...
   static function createModalWindowFago($name, $url, $options = []) {
      ...
      $out.=".done(function(data) {
               $('#massiveactionModalBody').html(data); 
               
               // Display Modal
               $('#massiveactionModal').modal('show'); 
            });";
      $out .= "};\n";
      $out .= "});";
      $out.= "</script>\n";
      
      $out.='<div class="modal fade" id="massiveactionModal" role="dialog" aria-labelledby="" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
               <div class="modal-content">
                  <div class="modal-header" style="padding: 10px; line-height:-1">
                     <h5 class="modal-title">'.$param['title'].'</h5>
                     <span  data-dismiss="modal" aria-label="Close" aria-hidden="true"><i class="fas fa-times fa-2x"></i></span>
                     
                  </div>

                  <div class="modal-body" id="massiveactionModalBody">
                  </div>
   ...
   static function createIframeModalWindow($domid, $url, $options = []) {
      ...
      $out  = "<div id=\"$domid\">";
      $out .= "<iframe  style='width:100%; height:100%; display:none; border:none'  id='Iframe$domid' class='iframe hidden'></iframe></div>";
      ...
      $out .= "<script type='text/javascript'>
         $(function() {
            $('#$domid').dialog({
               ...
               resizeable: true,
               open: function(ev, ui){
                  $('#Iframe$domid').css({ display: 'block' });
   ...
   static function createIframeModalWindow_old($domid, $url, $options = []) {
      ...
      $out='<div class="modal fade" id="m_modal_2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">';
         $out .='<div class="modal-dialog modal-lg">';
            $out .='<div class="modal-dialog" role="document">';
               $out.='<div class="modal-content">';
                  $out .='<div class="modal-header">';
                     $out .='	<h5 class="modal-title" id="exampleModalLongTitle">Modal title</h5>';
                     $out .='<button type="button" class="close" data-dismiss="modal" aria-label="Close">';
                        $out .='<span aria-hidden="true">&times;</span>';
                     $out .='</button>';
                  $out .='</div>';

                  $out .='<div class="modal-body" id="modal-body">';
                  $out .='</div>';

                  $out .='<div class="modal-footer">';
                     // $out .='<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>';
                     // $out .='<button type="button" class="btn btn-primary">Save changes</button>';
                  $out .='</div>';
               $out .='</div>';
            $out .='</div>';
         $out .='</div>';
   ...
   static function createTabs(
      ...
      $rand = mt_rand();
      if (count($tabs) > 0) {
         echo "<div  id='tabs$rand' class='tabs-content $mainclass $orientation'>";
         if (CommonGLPI::isLayoutWithMain()
            && !CommonGLPI::isLayoutExcludedPage()) {
            $orientation = 'horizontal';
         }
         echo "<ul class='container-lg nav nav-tabs tab-item ' role='tablist'>";
         $current = 0;
         $selected_tab = 0;
         foreach ($tabs as $key => $val) {
            ...
            echo "<li class='tab nav-item' ><a   class='nav-link '  title=\"".
               str_replace(["<sup class='tab_nb nav-link'>", '</sup>'], '', $val['title'])."\" ";
   ...
   static function updateItemOnEventFago($toobserve, $toupdate, $url, $parameters = [],
                                     $events = ["change"], $minsize = -1, $buffertime = -1,
                                     $forceloadfor = [], $display = true) {

      $output  = "<script type='text/javascript'>";
      $output .= "$(function() {";
      $output .= self::updateItemOnEventJsCodeFago($toobserve, $toupdate, $url, $parameters, $events,
                                               $minsize, $buffertime, $forceloadfor, false);
      $output .=  "});</script>";
      if ($display) {
         echo $output;
   ...
   static function updateItemOnSelectEventFago($toobserve, $toupdate, $url, $parameters = [], $events = ["change"],
                                             $display = true) {

      return self::updateItemOnEventFago($toobserve, $toupdate, $url, $parameters, $events,
                                       -1, -1, [], $display);
   }
   ...
   static function updateItemOnEventJsCodeFago($toobserve, $toupdate, $url, $parameters = [],
                                          $events = ["change"], $minsize = -1, $buffertime = -1,
                                          $forceloadfor = [], $display = true) {
      ...
      foreach ($zones as $zone) {
         foreach ($events as $event) {
            ...
            $output .= PluginServicesHtml::jsGetElementbyName(Html::cleanId($zone)).".on(
               '$event',
               function(event) {";
            $condition = '';
            if ($minsize >= 0) {
               $condition = PluginServicesHtml::jsGetElementbyName(Html::cleanId($zone)).".val().length >= $minsize ";
            }
            if (count($forceloadfor)) {
               foreach ($forceloadfor as $value) {
                  if (!empty($condition)) {
                  ...
                  $condition .= PluginServicesHtml::jsGetElementbyName(Html::cleanId($zone)).".val() == '$value'";
               }
            }
            if (!empty($condition)) {
               $output .= "if ($condition) {";
            }
            $output .= self::updateItemJsCodeFago($toupdate, $url, $parameters, $toobserve, false);
            if (!empty($condition)) {
               $output .= "}";
            }
            $output .=  "}";
   ...
   static function updateItemJsCodeFago($toupdate, $url, $parameters = [], $toobserve = "",
                                    $display = true) {
      // Ce controle est fait pour changer le contenu d'un dropdown 
      //(exemple des groupe d'asignation => si on cange un groupe d'assignation, on change aussi les données du champ asign to)
      if (isset($parameters['action']) && $parameters['action']==='setInputData') {
         $domId = $parameters['to_update'];
         $out = "$('[name=\"$domId\"]').parent().load('$url'\n";
      }else {
         $out = Html::jsGetElementbyID($toupdate).".load('$url'\n";
      }
      ...
      if (count($parameters)) {
         ...
         foreach ($parameters as $key => $val) {
            ...
            $regs = [];
            if (!is_array($val) && preg_match('/^__VALUE(\d+)__$/', $val, $regs)) {
               $out .=  PluginServicesHtml::jsGetElementbyName(Html::cleanId($toobserve[$regs[1]])).".val()";

            } else if (!is_array($val) && $val==="__VALUE__") {
               $out .=  PluginServicesHtml::jsGetElementbyName(Html::cleanId($toobserve)).".val()";
