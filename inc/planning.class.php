 ...
 * Planning Class
**/
class PluginServicesPlanning extends Planning {
   ...
   **/
   static function dropdownState($name, $value = '', $display = true, $options = []) {
      ...
      $values = [
         static::INFO => _n('Ouvert', 'Ouvert', 1),
         static::TODO => __('En cours'),
         static::DONE => __('Clos')
      ];

      return PluginServicesDropdown::showFromArray($name, $values, array_merge(['value'   => $value,
   ...
   **/
   ...
   static function showAddEventClassicForm($params = []) {
      ...
      if (isset($item->fields["begin"]) && ($item->fields["begin"] > 0)) {
         $params["begin"] = $item->fields["begin"];
      }

      if (isset($item->fields["Period"]) && ($item->fields["Period"] > 0)) {
         $params["Period"] = $item->fields["Period"];
      }
      ...
      if (isset($params["end"]) && !empty($params["end"])) {
         ...
         if ($display_dates) {
               PluginServicesHtml::showDateTimeField("plan[begin]", [
                  'value'      => $begin,
                  'maybeempty' => false,
                  'canedit'    => true,
                  'mindate'    => '',
   ...
   static function showAddEventClassicFormFago($params = []) {
      ...
      if (isset($params["end"]) && !empty($params["end"])) {
         ...
         $end = date("Y-m-d H:i:s", strtotime($begin)+HOUR_TIMESTAMP);
            ...
            PluginServicesDropdown::showTimeStamp("plan[_duration]", [
               'min'        => 0,
      ...
      //          <label class=''>"._x('Planning', 'Reminder')."</label>";
   ...
   static function showAddEventClassicFormModal($params = []) {
      global $CFG_GLPI;
      $item = new $params['itemtype']();
      $item->getFromDB($params['items_id']);
      if (isset($item->fields["begin"]) && ($item->fields["begin"] > 0)) {
      ...
      echo"<div class='form-group m-form__group row'>";
         if ($display_dates) {
            echo"<div class='col-lg-6'>";
               echo"<label>".__('Start date')."</label>";
               PluginServicesHtml::showDateTimeField("plan[begin]", [
                  'value'      => $begin,
                  'maybeempty' => false,
                  'canedit'    => true,
         ...
         echo"<div class='col-lg-6'>";
            ...
            $empty_label   =  PluginServicesDropdown::EMPTY_VALUE;
            $default_delay = $params['duration'] ?? 0;
            if ($display_dates) {
               $empty_label   = __('Specify an end date');
               $default_delay = floor((strtotime($end)-strtotime($begin))/$CFG_GLPI['time_step']/MINUTE_TIMESTAMP)*$CFG_GLPI['time_step']*MINUTE_TIMESTAMP;
