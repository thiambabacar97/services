...
class PluginServicesDropdown {
   ...
   **/
   static function show($itemtype, $options = []) {
      ...
      $output.= PluginServicesHtml::jsAjaxDropdown($params['name'], $field_id,
                                    $params['url'],
                                    $p);
      // Display comment
      if ($params['comments']) {
         $comment_id      = PluginServicesHtml::cleanId("comment_".$params['name'].$params['rand']);
         $link_id         = PluginServicesHtml::cleanId("comment_link_".$params['name'].$params['rand']);
         $kblink_id       = PluginServicesHtml::cleanId("kb_link_".$params['name'].$params['rand']);
         ...
         if (empty($comment)) {
            $comment = PluginServicesToolbox::ucfirst(
               sprintf(
                  __('Show %1$s'),
   ...
   static function showForSearchCriteria($itemtype, $options = []) {
   ...
   **/
   static function showGMT($name, $value = '') {
      ...
      self::showFromArray($name, $values, ['value' => $value]);
   }
   ...
   **/
   static function showItemTypeMenu($title, $optgroup, $value = '') {
      ...
      self::showFromArray('dpmenu', $values,
                              ['on_change'
   ...
   **/
   ...
   static function showHours($name, $options = []) {
      ...
      $p['value'] = $selected;
      return self::showFromArray($name, $values, $p);
   }
   ...
   **/
   static function showItemType($types = '', $options = []) {
      ...
      if (count($options)) {
         return self::showFromArray($params['name'], $options, [
            'value'               => $params['value'],
            'on_change'           => $params['on_change'],
   ...
   **/
   static function showNumber($myname, $options = []) {
      ...
      $field_id = PluginServicesHtml::cleanId("dropdown_".$myname.$p['rand']);
      ...
      $out   = PluginServicesHtml::jsAjaxDropdown($myname, $field_id,
   ...
   **/
   static function showTimeStamp($myname, $options = []) {
      ...
      return self::showFromArray($myname, $values,
                                     ['value'                => $params['value'],
                                            'display'             => $params['display'],
                                            'width'               => $params['width'],
                                            'display_emptychoice' => $params['display_emptychoice'],
   ...
   **/
   static function showFromArray($name, array $elements, $options = []) {
      ...
      if ($param['readonly']) {
         ...
         if ($param['class']) {
            $output .= ' class=" form-control m-select2 '.Html::entities_deep($param['class']).'"';
         }else{
            $output .= ' class=" form-control m-select2 "';
         }
      ...
      if (!$param['noselect2']) {
         // Width set on select
         $output .= PluginServicesHtml::jsAdaptDropdown($field_id, ['width' => $param["width"]]);
      }
      ...
      if ($params['value']
         ...
         if ($params['management_restrict'] == 2) {
            ...
            $values = [MANAGEMENT_UNITARY => __('Unit management'),
                            ...
                            self::showFromArray($params['name'], $values, ['value' => $params['value']]);
         } else {
            // Templates edition
   ...
   **/
   static function showListLimit($onchange = '', $display = true) {
      ...
      ksort($values);
      return Dropdown::showFromArray('glpilist_limit', $values,
                                 ['on_change' => $onchange,
                                       'value'     => $list_limit,
                                       'display'   => $display,
                                       'noselect2' => true,
   ...
   static function showOutputFormat() {
      ...
      $pdfportrait='-'.Search::PDF_OUTPUT_PORTRAIT;
      $pdflands='-'.Search::PDF_OUTPUT_LANDSCAPE;
      $slk='-'.Search::SYLK_OUTPUT;
      $csv='-'.Search::CSV_OUTPUT;
      echo"<button id='clickmassiveactionhidebutton' style='vertical-align: top; margin: 0 2px; border: 1px solid #dddfe5;'  type='button' class=' btn bg-secondary btn-sm btn-default '><i class='flaticon-cogwheel-2'></i></button>";
      echo'
         <div class="m-dropdown m-dropdown--inline m-dropdown--arrow m-dropdown--align-left" m-dropdown-toggle="hover" aria-expanded="true">
            <a href="#" style="border: 1px solid #dddfe5;" class="m-dropdown__toggle btn btn-default bg-secondary btn-sm dropdown-toggle"><i class="flaticon-download"></i></a>
            <div class="m-dropdown__wrapper" style="z-index: 101; width:100px">
               <span class="m-dropdown__arrow m-dropdown__arrow--left"></span>
               <div class="m-dropdown__inner">
                  <div class="m-dropdown__body">
                     <div class="m-dropdown__content">
                        <ul class="m-nav">
                           <li class="m-nav__item" id="b1">
                              <a href="" class="m-nav__link">
                              <input type="hidden" value="'.$pdflands.'" />
                                 <span class="m-nav__link-text">'. __('PDF').'</span>
                              </a>
                           </li>
                           <li class="m-nav__item" id="b2">
                              <a href="" class="m-nav__link">
                              <input type="hidden" value="'.$pdfportrait.'" />
                                 <span class="m-nav__link-text">'. __('PDF').'</span>
                              </a>
                           </li>
                           <li class="m-nav__item" id="b2">
                           <a href="" class="m-nav__link">
                           <input type="hidden" value="'.$csv.'" />
                              <span class="m-nav__link-text">'. __('CSV').'</span>
                           </a>
                        </li>
                        </ul>
                     </div>
      ...
      echo"
         <script>
            ...
            $('#b1, #b2').click(function(event){
               $('input[name=display_type]').val($(this).find('a>input').attr('value'));
   ...
   public static function getDropdownNumber($post, $json = true) {
      ...
      if (count($values)) {
         ...
         foreach ($tosend as $i) {
            $txt = $i;
            if (isset($post['unit'])) {
               $txt = PluginServicesDropdown::getValueWithUnit($i, $post['unit']);
            }
            $data[] = ['id' => $i,
               'text' => (string)$txt];
            $count++;
   ...
   static function dropdowElementform($item, $option=[]) {
      global  $CFG_GLPI;
         $rand  = mt_rand();
         $itm = new $item();
         $url = $itm->getFormURL();
         if (!empty($option['id'])) {
            $url =  $url.'?id='.$option['id'];
         }
         $pref_url = $CFG_GLPI["root_doc"]."/front/displaypreference.form.php?itemtype=Ticket";
         $field_id = Html::cleanId("dropdown_name".$rand);
         $output = "<span class='fa fa-plus-circle pointer' title=\"".__s('Add')."\"
                     onClick=\"".'add_'.$field_id."();\"
                     ><span class='sr-only'>" . __s('Add') . "</span></span>";
         $output .= PluginServicesAjax::createModalWindowFago('add_'.$field_id,  $pref_url, ['display' => true]);
         echo $output;
   }

   static function formModal($item, $option=[]) {
      $rand  = mt_rand();
      $itm = new $item();
      $url = $itm->getFormURL();
      if (!empty($option['id'])) {
      ...
      $field_id = Html::cleanId("dropdown_name".$rand);
      $output = "
         <button title=\"".__s('Add')."\"  onClick=\"".Html::jsGetElementbyID('add_'.$field_id).".dialog('open');\" style='font-weight: 500; font-size: 12px;' class='align-self-start btn btn-secondary btn-sm my-2 m-btn m-btn--custom m-btn--icon'> 
            <i class='flaticon-add-circular-button'></i> Nouveau
         </button>
      ";
      $output .= PluginServicesAjax::createIframeModalWindow('add_'.$field_id, $url, ['width' => 600, 'height' => 500, 'reloadonclose' => true]);
      echo $output;
   }
}
