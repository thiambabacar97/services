...
class PluginServicesProvider extends Provider {
   ...
   */
   ...
   static function bigNumberItem(CommonDBTM $item = null, array $params = []): array {
      ...
      $itemss = new $item();
      $list_params = explode('*', $params['label']);
      $attribute = $list_params[1];
      $data_param = [
         'label' => $list_params[0],
      ];
      
      $params = array_merge($default_params, $data_param);
      $i_table = $item::getTable();
      $raw = $itemss->getSearchOptionByField('field', $attribute,$i_table);
      ...
      // $itemtype = getItemTypeForTable($i_table);
      
      $customitem = 'PluginServices'.getItemTypeForTable($i_table);
      $itemtype = (class_exists($customitem)) ?  $customitem : getItemTypeForTable($i_table);
      $field = (isset($raw['field'])) ? $raw['field']: $attribute;
      // echo "The first field is : ".$field." and the second is: ".$attribute;
      
      $criteria = array_merge_recursive(
         [
            'SELECT' => ['COUNT' => "$i_table.id as number",
            "$i_table.$field as attribut",
            ],
            'FROM'   => $i_table,
            'GROUPBY' => "$i_table.$field",
            'ORDER'   => 'number DESC',
            'WHERE'  => $where
         ],
      ...
      foreach ($iterator as $result) {
         if($field == "priority" || $field == "urgency" || $field == "impact" || $field == "status" || $field == "type"){
            $val = $item->getSpecificValueToDisplay($field, $result['attribut']);
         }elseif($field == "users_id_recipient"){
            $table = getTableNameForForeignKeyField($field);
            $val = Dropdown::getDropdownName($table, $result['attribut']);
            $s_criteria = [
               'criteria' => [
                  [
                     'link'       => 'AND',
         ...
         }elseif($field == "users_id_lastupdater" || $field == "requesttypes_id" || $field == "users_id_recipient"){
            $table = getTableNameForForeignKeyField($field);
            // echo $i_table;
            $raw = PluginServicesRelatedlist::getSearchOptionByFields('datatype', 'dropdown', $i_table, $table);
            // print_r($raw);
            $val = Dropdown::getDropdownName($table, $result['attribut']);
            $s_criteria = [
               'criteria' => [
         ...
         }elseif($field == "requesttypes_id"){
            $table = getTableNameForForeignKeyField($field);
            // $test = $items->getSearchOptionsActors();
            // $raw = $items->getSearchOptionByField('linkfield', 'users_id_recipient', "glpi_users");
            $val = Dropdown::getDropdownName($table, $result['attribut']);
         ...
         $s_criteria['criteria'][0]['value'] = $result['attribut'];
         $data[] = [
            'number' => $result['number'],
            'label'  => $val,
            'url'    => PluginServicesToolbox::getItemTypeSearchURL($itemtype)."?".PluginServicesToolbox::append_params($s_criteria),
         ];
      }
      return [
         'data' => $data,
         'label'  => $item::getTypeName(1),
         'icon'   => $item::getIcon(),
      ];
   }
