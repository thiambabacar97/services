...
class PluginServicesSearch extends Search {
   ...
   static function showGenericSearch($itemtype, array $params) {
      ...
      if (class_exists($itemtype)) {
         ...
         $p['target']       = PluginServicesToolbox::getItemTypeSearchURL($itemtype);
      ...
      echo "<div style='margin:0; width: 100%;' id='searchcriteria' class='$main_block_class'>";
...
static function displayData(array $data) {
      ...
      if ($data['data']['count'] > 0) {
         ...
         if ($data['display_type'] == self::HTML_OUTPUT) {
            ...
            //    echo '<div class="d-flex justify-content-start mt-3" style="border-radius:5px; height:100%; box-shadow: 0px 1px 2px 1px #d2d2d2; padding:10px;">';
...
static function displayDataFagou(array $data, $addData) {
   global $CFG_GLPI;
   ...
   if ($data['data']['count'] > 0) {
      ...
      if ($data['itemtype'] != 'AllAssets') {
         $showmassiveactions = ($data['search']['showmassiveactions'] ?? true)
            && count(PluginServicesMassiveAction::getAllMassiveActions($data['item'],
                                                         $data['search']['is_deleted']));
      } else {
         $showmassiveactions = $data['search']['showmassiveactions'] ?? true;
      }
   ...
   static function showHeader($type, $rows, $cols, $fixed = 0) {
      ...
      switch ($type) {
         ...
         default :
            if ($fixed) {
               ...
               $out = "<div class='center'><table style='width:100%; margin-top:0;' border='0' class='m-0 tab_cadrehov table table-striped m-table'>\n";
            }
      }
      return $out;
   }
      ...
      if ($plug = isPluginItemType($itemtype)) {
         $out = PluginServicesPlugin::doOneHook(
            $plug['plugin'],
            'giveItem',
         ...
         switch ($table.'.'.$field) {
                     ...
                     if ((isset($data[$ID][$k]['name']) && ($data[$ID][$k]['name'] > 0))
                        ...
                        if ($itemtype == 'Ticket') {
                           if (isset($data[$ID][$k]['name'])
                              ...
                              $userdata["link"] = '/user/form/?id='.$data[$ID][$k]['name'];
                              $tooltip  = "";
                              if (Session::haveRight('user', READ)) {
            ...
            case "glpi_tickets_tickets.tickets_id_1" :
               ...
               for ($k=0; $k<$data[$ID]['count']; $k++) {
                  ...
                  if (($linkid > 0) && !isset($displayed[$linkid])) {
                     $text  = "<a ";
                     $text .= "href=\"".PluginServicesToolbox::getFormURLWithID($linkid)."\">";
            ...
            case "glpi_reservationitems.comment" :
               ...
               if (Session::haveRight('reservation', UPDATE)) {
                  return "<a title=\"".__s('Modify the comment')."\"
                           href='".PluginServicesToolbox::getFormURLWithID($data['refID'])."' >".$text."</a>";
               }
               return $text;
            ...
            case 'glpi_projectstates.name':
               ...
               if ($itemtype == 'ProjectState') {
                  $out =   "<a href='".PluginServicesToolbox::getFormURLWithID($data[$ID][0]["id"])."'>". $name."</a></div>";
               } else {
                  $out = $name;
               }
               return $out;
            ...
            case 'glpi_changes.name' :
               ...
               if (isset($data[$ID][0]['content'])
                  ...
                  $link  = PluginServicesToolbox::getFormURLWithID($data[$ID][0]['id'], false, $itemtype);
            ...
            case "itemlink" :
               ...
               for ($k=0; $k<$data[$ID]['count']; $k++) {
                  if (isset($data[$ID][$k]['id'])) {
                     ...
                     $page  = $CFG_GLPI['root_doc'].PluginServicesToolbox::getFormURLWithID($data[$ID][$k]['id'], false, $linkitemtype);
