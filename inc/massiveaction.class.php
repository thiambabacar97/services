 ...
 * @since 0.85
**/
class PluginServicesMassiveAction extends MassiveAction{
   ...
   **/
   function showDefaultSubForm() {
      echo"<div class='mt-2'>";
         echo PluginServicesHtml::submit(_x('button', 'Post'), ['name' => 'massiveaction']);
      echo"</div>";
   }
   ...
   static function showMassiveActionsSubForm(MassiveAction $ma) {
      ...
      switch ($ma->getAction()) {
         case 'update':
            if (!isset($ma->POST['id_field'])) {
               ...
               foreach ($itemtypes as $itemtype) {
                  ...
                  foreach (PluginServicesSearch::getCleanedOptions($itemtype, UPDATE) as $index => $option) {
                     ...
                     if (!is_array($option) || count($option) == 1) {
                        ...
                        if (($option['field'] != 'id')
                           ...
                           if (!isset($option['massiveaction']) || $option['massiveaction']) {
                              if (($show_all)
                                  || (($show_infocoms
                                       && PluginServicesSearch::isInfocomOption($itemtype, $index))
                                      || (!$show_infocoms
                                          && !PluginServicesSearch::isInfocomOption($itemtype, $index)))) {
               ...
               if (count($itemtypes) > 1) {
                  ...
                  $itemtype_choices = [-1 => PluginServicesDropdown::EMPTY_VALUE];
                  foreach ($itemtypes as $itemtype) {
               ...
               echo "<table width='100%' class='mt-3 mb-3'><tr>";
               ...
               if ($choose_field) {
                  echo "<td>";
                  $field_rand = PluginServicesDropdown::showFromArray('id_field', $options,
                                                         ['display_emptychoice' => true]);
                  echo "</td>";
               }
               if ($choose_itemtype) {
                  echo "<td>";
                  $itemtype_rand = PluginServicesDropdown::showFromArray('specialize_itemtype',
                                                            $itemtype_choices);
                  echo "</td>";
               }
               ...
               if ($choose_field) {
                  ...
                  PluginServicesAjax::updateItemOnSelectEvent("dropdown_id_field$field_rand",
               ...
               if ($choose_itemtype) {
                  ...
                  $params['specialize_itemtype'] = '__VALUE__';
                  $params['common_options']      = $common_options;
                  PluginServicesAjax::updateItemOnSelectEvent("dropdown_specialize_itemtype$itemtype_rand",
                                                "update_next_step$next_step_rand",
                                                $_SERVER['REQUEST_URI'], $params);
               }
            ...
            echo PluginServicesHtml::hidden('search_options', ['value' => $items_index]);
            echo PluginServicesHtml::hidden('field', ['value' => $fieldname]);
            echo "<br><br>\n";
            ...
            echo PluginServicesHtml::submit($submitname, ['name' => 'massiveaction']);
         ...
         case 'clone':
            ...
            echo "</td><tr>";
            echo "<td>".PluginServicesHtml::input("nb_copy", [
               'id'     => "nb_copy$rand",
               'value'  => 1,
               'type'   => 'number',
               'min'    => 1
         ...
         case 'add_transfer_list':
            ...
            echo "<br><br>";
            echo PluginServicesHtml::submit(_x('button', 'Add'), ['name' => 'massiveaction']);
         ...
         case 'amend_comment':
            ...
            echo ("<br><br>");
            echo PluginServicesHtml::submit(__('Update'), [
               'name' => 'massiveaction'
            ]);
         ...
         case 'add_note':
            ...
            echo ("<br><br>");
            echo PluginServicesHtml::submit(_sx('button', 'Add'), [
               'name' => 'massiveaction'
            ]);
   ...
   **/
   function itemDone($itemtype, $id, $result) {
      ...
      if (is_countable($this->remainings[$itemtype]) && count($this->remainings[$itemtype]) == 0) {
