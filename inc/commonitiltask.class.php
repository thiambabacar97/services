...
class PluginServicesCommonITILTask extends CommonITILTask {
   ...
   **/
   ...
   function prepareInputForUpdate($input) {
      ...
      if (isset($input["plan"])) {
         ...
         PluginServicesPlanning::checkAlreadyPlanned($input["users_id_tech"], $input["begin"], $input["end"],
   ...
   function post_addItem() {
      ...
      if (isset($this->fields["begin"]) && !empty($this->fields["begin"])) {
         PluginServicesPlanning::checkAlreadyPlanned($this->fields["users_id_tech"], $this->fields["begin"],
                                       $this->fields["end"],
   ...
   function post_getEmpty() {
      ...
      // Default is todo
      $this->fields['state'] = PluginServicesPlanning::TODO;
   ...
   **/
   static function genericPopulatePlanning($itemtype, $options = []) {
      ...
      if (!$options['display_done_events']) {
         $WHERE[] = ['OR' => [
            $item->getTable() . ".state"  => PluginServicesPlanning::TODO,
            [
               'AND' => [
                  $item->getTable() . '.state'  => PluginServicesPlanning::INFO,
   ...
   **/
   static function genericDisplayPlanningItem($itemtype, array $val, $who, $type = "", $complete = 0) {
      ...
      if (isset($val["state"])) {
         $html.= "<span>";
         $html.= PluginServicesPlanning::getState($val["state"]);
         $html.= "</span>";
      }
      $html.= "<div>";
   ...
   function showInObjectSumnary(CommonITILObject $item, $rand, $showprivate = false) {
      ...
      if ($canview) {
         ...
         switch ($this->fields['state']) {
            case PluginServicesPlanning::INFO :
               echo Html::image($CFG_GLPI['root_doc']."/pics/faqedit.png",
                                ['title' =>_n('Ouvert', 'Ouvert', 1)]);
               break;

            case PluginServicesPlanning::TODO :
               if (empty($this->fields['begin'])) {
            ...
            case PluginServicesPlanning::DONE :
         ...
         if (empty($this->fields["begin"])) {
            if (isset($this->fields["state"])) {
               echo PluginServicesPlanning::getState($this->fields["state"])."<br>";
            }
            ...
            if (isset($this->fields["state"])) {
               ...
               echo PluginServicesPlanning::getState($this->fields["state"])."</td></tr>";
            }
   ...
   **/
   function showForm($ID, $options = []) {
      ...
      if (isset($this->fields["state"])) {
         echo "<div class='fa-label'>
            <i class='fas fa-tasks fa-fw'
               title='".__('Status')."'></i>";
         PluginServicesPlanning::dropdownState("state", $this->fields["state"], true, ['rand' => $rand_state]);
         echo "</div>";
      }
      ...
      if (!empty($this->fields["begin"])) {
         ...
         if (Session::haveRight('planning', PluginServicesPlanning::READMY)) {
   ...
   function showMassiveActionAddTaskForm() {
      ...
      echo "<td>";
      PluginServicesPlanning::dropdownState("state", $_SESSION['glpitask_state']);
      echo "</td>";
      echo "</tr>";
   ...
   public static function getTaskList($status, $showgrouptickets, $start = null, $limit = null) {
      ...
      switch ($status) {
         case "todo" : // we display the task with the status `todo`
            $prep_req['WHERE'][self::getTable() . '.state'] = PluginServicesPlanning::TODO;
            break;
         ...
         switch ($status) {
            case "todo" :
               ...
               $options['criteria'][] = [
                  ...
                  'searchtype' => 'equals',
                  'value'      =>  PluginServicesPlanning::TODO,
                  'link'       => 'AND',
               ];
